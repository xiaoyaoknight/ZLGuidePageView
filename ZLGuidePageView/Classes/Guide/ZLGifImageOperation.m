...
@implementation ZLGifImageOperation
...
- (id)initWithFrame:(CGRect)frame gifImagePath:(NSString *)gifImagePath {
    ...
    if (self) {
        ...
        self.gif = CGImageSourceCreateWithURL((CFURLRef)[NSURL fileURLWithPath:gifImagePath], (CFDictionaryRef)self.gifProperties);
        self.count =CGImageSourceGetCount(self.gif);
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.06 target:self selector:@selector(play) userInfo:nil repeats:YES];/**< 0.12->0.06 */
        [self.timer fire];
    }
    return self;
}
...
- (id)initWithFrame:(CGRect)frame gifImageData:(NSData *)gifImageData {
    ...
    if (self) {
        self.gifProperties = [NSDictionary dictionaryWithObject:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:0] forKey:(NSString *)kCGImagePropertyGIFLoopCount] forKey:(NSString *)kCGImagePropertyGIFDictionary];
        self.gif = CGImageSourceCreateWithData((CFDataRef)gifImageData, (CFDictionaryRef)self.gifProperties);
        self.count =CGImageSourceGetCount(self.gif);
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.06 target:self selector:@selector(play) userInfo:nil repeats:YES];/**< 0.12->0.06 */
        [self.timer fire];
    }
